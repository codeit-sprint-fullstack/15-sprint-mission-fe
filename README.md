...
계속해서 고칠 게 나오는 걸 보면서 제가 아직 많이 미숙하다는 걸 느꼈습니다.
작성한 코드를 보면서 어떻게 하면 효율적이고 보기 좋을지 고민하는 과정이 재밌었습니다.
화면의 콘텐츠들이 중앙 정렬이 안 되는 문제와, 화면이 줄어들 때 콘텐츠가 요구사항과 다르게 또는 보기 안 좋게 보이는 걸 고치는 데 시간을 많이 사용했습니다.
아직 시맨틱 태그와 BEM 방식이 익숙하지 않아서 여러 가지 시행착오를 겪고 수정하고 또 수정했습니다. `<h1~h6>`, `<main>`, `<footer>` 태그의 올바른 사용법을 특히 많이 고민했습니다.

---
### 미션 3였던거 — 유효성 검사
- [x] 공통 로직 모듈화
- [x] 유효성 검사 (login.html, signup.html)
- [ ] 로그인/회원가입 실패 팝업
- [x] 비밀번호 표시/숨기기
- [ ] SNS 공유 메타 태그
